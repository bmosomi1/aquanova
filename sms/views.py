...
def water_expense_category(request,category_id):
    expense = WaterExpenses.objects.filter(category=category_id)
    context = {
        'expenses': expense
    }
    return render(request, 'sms/water_expenses.html', context)
...
def disconnection_reminder(request, client_id):
    ...
    client_message = dear  +  client_name + final + client_balance + payment + account_client  + disconnection + current_date.strftime("%m:%d:%Y") + dis
...
def meter_readings_report(request):
    ...
    if request.method == 'POST':
        ...
        if report_value=='1':
            ...
            summary_sheet.append(('OLEMAX WATER CLIENTS','NAMES'))
